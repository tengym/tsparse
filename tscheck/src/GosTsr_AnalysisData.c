...
#define FREE_ALL_LIST(ptr) do{	\
	TS_PACKAGE_INFO *prePackage = GOSTSR_NULL;	\
	TS_PACKAGE_INFO *pCurPackage = (TS_PACKAGE_INFO *)ptr;	\
...
static TS_PACKAGE_INFO *PackageHead[TSERROR_CHANNELID_MAX][TABLE_NUMBER];
...
static GOSTSR_PSISI_PAT_S gstPatInfo[TSERROR_CHANNELID_MAX];
...
{
	GOSTSR_S32 i =0;
	memset(&gstPatInfo, 0, sizeof(GOSTSR_PSISI_PAT_S) * TSERROR_CHANNELID_MAX);

	for(i = 0; i < TSERROR_CHANNELID_MAX;i++)
	{
	    	gstPatInfo[i].u8Version = 0xff;	
	}

	return GOSTSR_SUCCESS;
}
GOSTSR_S32 GosTsr_AnalysisData_ReInitPatInfo(GOSTSR_U8 chanID)
{
	memset(&gstPatInfo[chanID], 0, sizeof(GOSTSR_PSISI_PAT_S));
	gstPatInfo[chanID].u8Version = 0xff;	
	
	return GOSTSR_SUCCESS;
}

GOSTSR_S32 GosTsr_AnalysisData_getPatInfo(GOSTSR_U8 chanID,GOSTSR_PSISI_PAT_S *patInfo)
{
	if(gstPatInfo[chanID].u8Version != patInfo->u8Version)
    {
        memcpy(&gstPatInfo[chanID],patInfo,sizeof(GOSTSR_PSISI_PAT_S));
    }
...
GOSTSR_S32 GosTsr_AnalysisData_TsEnvInit()
{
	...
	for(j = 0; j < TSERROR_CHANNELID_MAX; j++)
	{
	    for (i = 0; i < TABLE_NUMBER; i++)
	    {
	        /*free all package list member*/
	        pCurPackage = PackageHead[j][i];
	        while (pCurPackage != GOSTSR_NULL)
	        {
	            prePackage = pCurPackage;
	            pCurPackage = pCurPackage->next;
...
GOSTSR_S32 GosTsr_AnalysisData_TsEnvReInit(GOSTSR_U8 chanID)
{
	TS_PACKAGE_INFO *pCurPackage = GOSTSR_NULL;
    	TS_PACKAGE_INFO *prePackage = GOSTSR_NULL;
	GOSTSR_U8 i = 0;
	...
	{
		/*free all package list member*/
		pCurPackage = PackageHead[chanID][i];
		while (pCurPackage != GOSTSR_NULL)
		{
			prePackage = pCurPackage;
			pCurPackage = pCurPackage->next;
		...
		PackageHead[chanID][i] = GOSTSR_NULL;
	}
	GosTsr_AnalysisData_ReInitPatInfo(chanID);
	
	return GOSTSR_SUCCESS;
}

GOSTSR_S32 GosTsr_AnalysisData_CallBack(GOSTSR_U8 chanID,TS_SECTION_INFO *tsSectionInfo)
{
    GOSTSR_U8 i = 0;
    ...
    for (i = 0; i < ANALYSIS_CALLBACKFUNCTION_NUM; i++)
    {
        ...
        {
            callBackFunc[i](chanID,tsSectionInfo);
        }
    }
...
GOSTSR_S32 GosTsr_AnalysisData_PacketType(GOSTSR_U32 *pu32PacketLen, GOSTSR_U32 u32DatatLen,GOSTSR_U8 *pData)
{
	GOSTSR_U8 chr = '\0';
	GOSTSR_BOOL checkFlag = GOSTSR_FALSE;
	GOSTSR_U32 CurOffset = 0;
	GOSTSR_U32 PreOffset = 0;
	GOSTSR_U32 packageLen = 0;
	GOSTSR_U32 pos =0;
	GOSTSR_U8 *tsData = GOSTSR_NULL;
	GOSTSR_U8 *pTempData = pData;
	
	if((pu32PacketLen == GOSTSR_NULL) || (pData == GOSTSR_NULL))
	{
		return GOSTSR_FAILURE;
	}
	do
	{
		...
		if (chr == SYNC_BYTE)
		{
			...
			{
				if (((CurOffset - PreOffset) == TS_LENGTH_188) || ((CurOffset - PreOffset) == TS_LENGTH_204))
				{
					CurOffset = PreOffset;
	...
	tsData = (GOSTSR_U8 *)OS_Packet_AllocMemory(TS_LENGTH_204 + 1);
	...
	memcpy(tsData, pTempData, TS_LENGTH_204 + 1);
...
GOSTSR_S32 GosTsr_AnalysisData_LinkPackage(GOSTSR_U8 tableID, TS_PACKAGE_INFO *PackageHead_Temp, GOSTSR_U8 lastDataLen, GOSTSR_U8 *lastData)
{
    ...
    curSectionInfo = (TS_SECTION_INFO *)OS_Packet_AllocMemory(sizeof(TS_SECTION_INFO));
    ...
    curSectionInfo->sectionData = (GOSTSR_U8 *)OS_Packet_AllocMemory(curSectionLength);
    if(GOSTSR_NULL == curSectionInfo->sectionData)
    {
    	OS_Packet_FreeMemory(curSectionInfo);
        return  GOSTSR_FAILURE;
    }
    ...
    {
        GosTsr_AnalysisData_CallBack(curSectionInfo->stErrorInfo.chanID,curSectionInfo);
    }
	
#if TSERROR_CHECK
	#if TSERROR_TWO_LEVEL
    ...
    if (GOSTSR_NULL != curSectionInfo)
    {
        if (GOSTSR_NULL != curSectionInfo->sectionData)
        {
           OS_Packet_FreeMemory(curSectionInfo->sectionData);
        }
        OS_Packet_FreeMemory(curSectionInfo);
    }
...
GOSTSR_S32 GosTsr_AnalysisData_TableID(GOSTSR_U8 tableID,const void *data, TS_HEAD_INFO *tsHeadInfo)
{
    ...
    if ((GOSTSR_NULL == data) || (GOSTSR_NULL == tsHeadInfo))
    {
        return  GOSTSR_FAILURE;
    }
	...
	if (curTableID != tableID)
        ...
        for (i = 0; i < TABLE_NUMBER; i++)
        {
            if (PackageHead[tsHeadInfo->stErrorInfo.chanID][i] == GOSTSR_NULL)
            {
                continue;
            }

            if (PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->PID == tsHeadInfo->ts_pid)
            {
                allSectionLength = 0;

                pCurPackage = PackageHead[tsHeadInfo->stErrorInfo.chanID][i];
                while (GOSTSR_NULL != pCurPackage)
                {
                    allSectionLength += pCurPackage->packageLen;
                    pCurPackage = pCurPackage->next;
                ...
                if((allSectionLength + pointFieldLength) >= PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->sectionLen)
                {
                    GosTsr_AnalysisData_LinkPackage(tableID,PackageHead[tsHeadInfo->stErrorInfo.chanID][i],pointFieldLength,frontSectionData);
                }
		/*free all list memory!*/
		FREE_ALL_LIST(PackageHead[tsHeadInfo->stErrorInfo.chanID][i]);
        ...
        {
            curSectionInfo = (TS_SECTION_INFO *)OS_Packet_AllocMemory(sizeof(TS_SECTION_INFO));
			if(curSectionInfo == GOSTSR_NULL)
			{
				return GOSTSR_FAILURE;
			}
			memset(curSectionInfo, 0x00, sizeof(TS_SECTION_INFO));
            curSectionInfo->sectionLength = curSectionLength;
			...
			memcpy(&curSectionInfo->stErrorInfo,&tsHeadInfo->stErrorInfo,sizeof(TR101290_ERROR_S));
         	...
         	curSectionInfo->sectionData = (GOSTSR_U8 *)OS_Packet_AllocMemory(curSectionLength);
			if(curSectionInfo->sectionData == GOSTSR_NULL)
			{
				OS_Packet_FreeMemory(curSectionInfo);
				return GOSTSR_FAILURE;
			}
        ...
        else /*create a new section package list head*/
        {
            ...
            if (TABLE_NUMBER != i)
            {
                PackageHead[tsHeadInfo->stErrorInfo.chanID][i] = (TS_PACKAGE_INFO*)OS_Packet_AllocMemory(sizeof(TS_PACKAGE_INFO));
				if(PackageHead[tsHeadInfo->stErrorInfo.chanID][i] != GOSTSR_NULL)
				{
					PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->next = GOSTSR_NULL;
	                PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->packageLen = remainLen;
	                memset(PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->packageData, 0x00, sizeof(PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->packageData));
	                memcpy(PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->packageData, pcurData, remainLen);
	                PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->PID = tsHeadInfo->ts_pid;
	                PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->number = tsHeadInfo->counter;
	                PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->sectionLen = curSectionLength;
	                PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->tableID = tableID;
					memcpy(&PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->stErrorInfo,&tsHeadInfo->stErrorInfo,sizeof(TR101290_ERROR_S));
    ...
    else
    {
        ...
        packageAllLen = 0;
        pCurPackage = PackageHead[tsHeadInfo->stErrorInfo.chanID][i];
        prePackage = PackageHead[tsHeadInfo->stErrorInfo.chanID][i];
        while (pCurPackage != GOSTSR_NULL)
        {
        ...
        pCurPackage = (TS_PACKAGE_INFO*)OS_Packet_AllocMemory(sizeof(TS_PACKAGE_INFO));
        ...
        pCurPackage->sectionLen = PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->sectionLen;
        pCurPackage->tableID = PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->tableID;
		memcpy(&pCurPackage->stErrorInfo,&tsHeadInfo->stErrorInfo,sizeof(TR101290_ERROR_S));
        /*update total received packages length*/
        packageAllLen += pCurPackage->packageLen;

        if (packageAllLen >= PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->sectionLen)
        {
            packageAllLen = 0;
            j = PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->number;

            /*check: whether all packages of one section have arrived or no?*/
            while(packageAllLen < PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->sectionLen)
            {
                pCurPackage = PackageHead[tsHeadInfo->stErrorInfo.chanID][i];
                while (pCurPackage != GOSTSR_NULL)
                {
            ...
            {
                tableID = PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->tableID;
                curSectionLength = PackageHead[tsHeadInfo->stErrorInfo.chanID][i]->sectionLen;
                curSectionInfo = (TS_SECTION_INFO *)OS_Packet_AllocMemory(sizeof(TS_SECTION_INFO));
				if(curSectionInfo == GOSTSR_NULL)
				{
					return GOSTSR_FAILURE;
                ...
                while (packageAllLen < curSectionLength)
                {
                    ...
                    if (GOSTSR_NULL != pCurPackage)
                    {
                        ...
                        if (prePackage == PackageHead[tsHeadInfo->stErrorInfo.chanID][i]) /*if current package is head package*/
                        {
                            PackageHead[tsHeadInfo->stErrorInfo.chanID][i] = prePackage->next;
							OS_Packet_FreeMemory(prePackage);
                        }
                        else
                        {
                            prePackage->next = pCurPackage->next;
							OS_Packet_FreeMemory(pCurPackage);
                        }
...
#if TSERROR_CHECK
			#if TSERROR_TWO_LEVEL
				/*Tow Level Error Check,若CRC错误(返回succeed)则不进行其他错误检测*/
				if(TsErrorCheck_TwoLevel_checkCrcError(&curSectionInfo->stErrorInfo,curSectionInfo->tableID, crc32, calCrc32) !=  GOSTSR_SUCCESS)
				{	
					#if TSERROR_THREE_LEVEL
					TsErrorCheck_ThreeLevel_SectionsTimeError(curSectionInfo);
...
GOSTSR_S32 GosTsr_AnalysisData_AdaptationInfo(TS_HEAD_INFO *desData, const void *data, TS_ADAPT_INFO *adaptInfo)
{
    ...
    if ((GOSTSR_NULL == desData) || (GOSTSR_NULL == data) || (GOSTSR_NULL == adaptInfo))
    {
        return GOSTSR_FAILURE;
    }
	...
	{	
		TsErrorCheck_OneLevel_ContinuityCounterError(&desData->stErrorInfo,desData->ts_pid,desData->counter,desData->adapter_control,0);//最后一个参数还未获取到，其值为0
		return GOSTSR_SUCCESS;
	}
	#endif
#endif
...
GOSTSR_S32 GosTsr_AnalysisData_PESInfo(TS_HEAD_INFO *desData,const void *data, TS_PES_INFO *pesInfo)
{
    ...
    if ((GOSTSR_NULL == data) || (GOSTSR_NULL == pesInfo) || (GOSTSR_NULL == desData))
    {
        return GOSTSR_FAILURE;
    }
...
GOSTSR_S32 GosTsr_AnalysisData_PSI(const void *data, TS_HEAD_INFO *tsHeadInfo)
{
	...
	for(i = 0;  i < gstPatInfo[tsHeadInfo->stErrorInfo.chanID].u16NbElements;i++)
	{
		if(gstPatInfo[tsHeadInfo->stErrorInfo.chanID].astElement[i].u16Pid == tsHeadInfo->ts_pid)
		{
...
GOSTSR_S32 GosTsr_AnalysisData_PAT(TS_SECTION_INFO *sectionInfo, GOSTSR_PSISI_PAT_S *patInfo)
{
    ...
    GosTsr_AnalysisData_getPatInfo(sectionInfo->stErrorInfo.chanID,patInfo);
...
GOSTSR_S32 GosTsr_AnalysisData_BAT(TS_SECTION_INFO *sectionInfo, GOSTSR_PSISI_BAT_S *batInfo)
{
	...
	if(batInfo->astDescriptor[i].Data == GOSTSR_NULL)
	{
		return GOSTSR_FAILURE;
	}
...
static GOSTSR_S32 AnalysisData_ParseDesc_Sdt(GOSTSR_U8 *pDescData, GOSTSR_S16 s16DescLen, char *servicename)
{
	GOSTSR_S16	s16DescriptorLen = s16DescLen;
	GOSTSR_U8 	*pData = pDescData;
	GOSTSR_U8	u8DescTag = 0;
	GOSTSR_U8	u8DescLen= 0;
	GOSTSR_U8	u8ProviderNameLen= 0;
	GOSTSR_U8	u8ServiceNameLen= 0;
	
	if((GOSTSR_NULL == pDescData) || (0 == s16DescLen) || (GOSTSR_NULL == servicename))
	{
		return GOSTSR_FAILURE;
	}

	while(s16DescriptorLen > 0)
	{
		u8DescTag = pData[0];
		u8DescLen = pData[1];
		pData += 2;
		if(u8DescTag == GOS_PSISI_DESCRIPTOR_SERVICE)
		{
			pData += 1;
		
			u8ProviderNameLen = pData[0];
			pData += 1;

			/*Provider name*/
			pData += u8ProviderNameLen;

			u8ServiceNameLen = pData[0];
			pData += 1;
			
			/*service name*/
			memcpy(servicename, pData, u8ServiceNameLen);
            printf("!!!!!=======name:%s\n", pData);
            pData += u8ServiceNameLen;
			
			return GOSTSR_SUCCESS;
		}

		s16DescriptorLen -= u8DescLen;
		pData +=  u8DescLen;
	}
...
GOSTSR_S32 GosTsr_AnalysisData_getServiceName(char *servicename, GOSTSR_U16 *serviceID, TS_SECTION_INFO *pSectionInfo)
{
	GOSTSR_S32	s32Ret = GOSTSR_FAILURE;
	GOSTSR_S16 	s16Elements_Len = 0;
	GOSTSR_S16 	s16Descriptor_Len = 0;
	GOSTSR_U8	*pDescriptor_Data = GOSTSR_NULL;
	
	GOSTSR_U8 	  *pData = pSectionInfo->sectionData;
	GOSTSR_U32	u32SectionLen = pSectionInfo->sectionLength;
	
	if((GOSTSR_NULL == pSectionInfo) || (GOSTSR_NULL == servicename) || (GOSTSR_NULL == serviceID))
	{
		return GOSTSR_FAILURE;
	}
	...
	s16Elements_Len = (u32SectionLen - 8 - 3 - 4) ;	

	while(s16Elements_Len > 0)
	{
        *serviceID = pData[0] * 256 + pData[1];
		pData += 3;
		
		s16Descriptor_Len = (pData[0] & (~(BIT8 | BIT7 | BIT6 | BIT5)))*256 + pData[1];
		pData += 2;	

		if(s16Descriptor_Len > 0)
		{
			/*jump to descritor,and store discriptor*/
			pDescriptor_Data = (GOSTSR_U8 *)malloc(sizeof(GOSTSR_U8) * s16Descriptor_Len);
			if(GOSTSR_NULL == pDescriptor_Data)
			{
				return GOSTSR_FAILURE;
			}
			memset(pDescriptor_Data, 0x00, s16Descriptor_Len);
            memcpy(pDescriptor_Data, pData, s16Descriptor_Len);

			s32Ret = AnalysisData_ParseDesc_Sdt(pDescriptor_Data, s16Descriptor_Len, servicename);

			if(pDescriptor_Data)
			{
				free(pDescriptor_Data);
				pDescriptor_Data = GOSTSR_NULL;
			}
		}
		
		pData += s16Descriptor_Len;
		
		s16Elements_Len -= (5 + s16Descriptor_Len);
	}

	return GOSTSR_SUCCESS;
}

GOSTSR_S32 GosTsr_AnalysisData_PAT_Free(GOSTSR_PSISI_PAT_S *patInfo)
{
...
GOSTSR_S32 GosTsr_AnalysisData_CAT_Free(GOSTSR_PSISI_CAT_S *catInfo)
{
	GOSTSR_U32 i = 0;
	...
	{
		if (GOSTSR_NULL == catInfo->astDescriptor[i].Data)
		{
			continue;
		}

		free(catInfo->astDescriptor[i].Data);
		catInfo->astDescriptor[i].Data = GOSTSR_NULL;
	}
	
	return GOSTSR_SUCCESS;
}

GOSTSR_S32 GosTsr_AnalysisData_PMT_Free(GOSTSR_PSISI_PMT_S *pmtInfo)
{
	GOSTSR_U32 i = 0, j= 0;
	...
	for (i = 0; i < pmtInfo->u16NbDescriptors; i++)
	{
		if (GOSTSR_NULL == pmtInfo->astDescriptor[i].Data)
		{
			continue;
		}

		free(pmtInfo->astDescriptor[i].Data);
		pmtInfo->astDescriptor[i].Data = GOSTSR_NULL;
	}

	for (i = 0; i < pmtInfo->u16NbElements; i++)
	{
		for (j = 0; j < pmtInfo->astElement[i].u16NbDescriptors; j++)
		{
			if (GOSTSR_NULL == pmtInfo->astElement[i].astDescriptor[j].Data)
			{
				continue;
			}
			free(pmtInfo->astElement[i].astDescriptor[j].Data);
			pmtInfo->astElement[i].astDescriptor[j].Data = GOSTSR_NULL;
		}
	}
